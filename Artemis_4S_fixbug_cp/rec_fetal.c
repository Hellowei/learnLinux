...
static INT16  sLastWavedata[4] = {-1, -1, -1,-1};
...
INT32 RecFetalUpdataBuff (PRECEVENTCLASS this, int source)
{
	...
	for (i = 0; i < pFhrCtrl->recRate; i++)
	{
		...
		RecAutoRealSave[i] = p->afm;//加上打印胎儿活动曲线
...
{	
	INT	i;
	UINT8  *p;
	INT32 databuf[4][REC_FHR_CURVE_POINTS];
	...
	RecScaleWave(top[3], bot[3], 0, 40, databuf[3], data_num);
	if (FhrConfig.mode != 1)
	{
	...
	RecBuildFetalGraph (this, databuf[3], data_num, 3);
