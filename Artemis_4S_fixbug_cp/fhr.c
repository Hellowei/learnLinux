...
{	//2015-06-13@vinYin 修复大字体界面退出演示模式，标准界面仍显示演示模式
    if (MonitorConfig.faceType == FACE_STD 
        || MonitorConfig.faceType == FACE_STD_SINGLE|| MonitorConfig.faceType == FACE_BIGFONT)
    {
        ReInitFhrGphMem(!MonitorConfig.nightModeOn);
        if (!isDemo)
        {
...
VOID EraseWavesBuffer(VOID)
{
	...
	for (j = 0; j < WAVE_WIDTH; j++)
	{				
		this->yPos[WAVE_FHR_CH1][j] = -1;
		this->yPos[WAVE_FHR_CH2][j] = -1;
		this->yPos[WAVE_TOCO][j] = -1;
		this->yPos[WAVE_AFM][j] = -1;
		ZeroMemory(&this->marks[j],sizeof(MARKEVENT));///修复切换演示模式标记再次出现问题
	}
    this->pHead     = FetalWaveBuffHeadPtr;
    this->pTail     = FetalWaveBuffTailPtr;
    this->pPage     = FetalWaveBuffWritePtr;
