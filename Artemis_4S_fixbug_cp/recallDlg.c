	...
	DrawFetalRecallFhrWave(HWND hWnd, PFETALDATA data, INT32 index, INT32 len)
{
	...
	for (i = 0; i < loops; i++)
	{
		...
		n = ((ndx / FETAL_SAMPLE_RATE) + index)% DATA_SAVE_LEN;
		// 显示波形
		m = (ndx & 0x03);
		...
		oldPox2 = newPox2;
		// 显示事件
		if (data[n].event && lastEventPos != n)
		{	
			PutRecallEvent(hdc, i, data[n].event);
			lastEventPos = n;
		}
	...
	DrawFetalRecallTocoWave(HWND hWnd, PFETALDATA data, INT32 index, INT32 len)
{
	...
	for (i = 0; i < loops; i++)
	{
		...
		//自动胎动 手动胎动 宫压归零的显示
		if (last_pos == n)//相同位置只显示一次
			continue;
		else
			last_pos = n;
			
		if (data[n].wave[0].fmbits || data[n].wave[1].fmbits
			|| data[n].wave[2].fmbits || data[n].wave[3].fmbits )
			mark |= 0x01;
		if (mark)
		{	
		...
		if (data[n].wave[0].afmbits || data[n].wave[1].afmbits
			|| data[n].wave[2].afmbits || data[n].wave[3].afmbits )
			mark |= 0x04;
		if (mark)
		{	
			PutRecallFetalMark(hdc, i, mark);
		...
		if (data[n].wave[0].zerobits || data[n].wave[1].zerobits
			|| data[n].wave[2].zerobits || data[n].wave[3].zerobits )
			mark |= 0x02;
		if (mark)
		{	printf("zero is %d\n",i,mark);
			PutRecallFetalMark(hdc, i, mark);
...
static BOOL ChoseAnalysDlg_OnCommand(HWND hWnd, DWORD choiceID)
{
	...
	{	case IDC_ANALY_SETUP_SONWINDOW_BUT1:
		...
		case IDC_ANALY_SETUP_SONWINDOW_BUT4:
			analyType = (INT)(choiceID - IDC_ANALY_SETUP_SONWINDOW_BUT1 + 1); 
			CheckDlgButton(hWnd,choiceID,TRUE);//相应的单选框设选中状态
			if(FhrConfig.defaultAnalyType != analyType)
			{
				FhrConfig.defaultAnalyType = analyType;
				Save_Config(CFG_KEYID_DEFAULTANALYTYPE);
			}
			break;
		case IDC_ANALY_SETUP_SONWINDOW_EXIT_B://按下确认
			EndDialog(hWnd, IDOK);
			break;
		default:
			return 0;
...
VOID SetDefaultAnalyDialog(HANDLE hOwner)
{
	...
	//带文字的四个小方格
		...
		{ CTRL_BUTTON,  WS_VISIBLE | WS_TABSTOP | BS_CHECKBOX, WS_EX_NONE,
			DLG_START_X,DLG_START_Y+KEY_HEIGHT*3,STATIC_HEIGHT+140,STATIC_HEIGHT,
			IDC_ANALY_SETUP_SONWINDOW_BUT4, STR_DLG_ANLS_CST, 0L, 0L },
	
		{ CTRL_BUTTON,  WS_VISIBLE | WS_TABSTOP | BS_ICON, WS_EX_NONE,
			DLG_WIDTH - EXIT_ICON_WIDTH - DLG_OFFSET,-(DLG_MENU_HIGHT + EXIT_ICON_HIGH) / 2,EXIT_ICON_WIDTH,EXIT_ICON_HIGH,
			IDC_ANALY_SETUP_SONWINDOW_EXIT_B, 0, 0L, 0L }
	};	

	DLGTEMPLATE dlg_data = {
	   WS_ABSSCRPOS | WS_CAPTION | WS_BORDER | WS_MODALDLG, WS_NONE, 
	   0, 0, DLG_WIDTH, DLG_HEIGHT,  STR_DLG_ANALY_PRINT, 0, NULL, 0L,
	};
	...
	SetCtrlDataAddData2(ctrl_data, loops, IDC_ANALY_SETUP_SONWINDOW_EXIT_B, 
						(UINT32)&btn_exit);
