...
BOOL FactoryMaintainDlgOnCommand(HWND hWnd, WPARAM wParam)
{
	...
	switch (LOWORD(wParam))
	{
		...
		case IDC_FACTORY_MAINTAIN_ENBALALY_B:
			...
			break;
		//按下宫压较对
		case IDC_FACTORY_MAINTAIN_TOCO_CHECK:
			FetalSetMoudle(FALSE,TRUE);
			break;
...
VOID FactoryMaintainDialog(HANDLE hOwner)
{
	...
	enum __dialog_size
	{
		...
		DLG_ROW1_Y	= DLG_OFFSET + (CTRL_HEIGHT-h) * 0,
		DLG_ROW2_Y	= DLG_OFFSET + (CTRL_HEIGHT-h) * 1,
		DLG_ROW3_Y	= DLG_OFFSET + (CTRL_HEIGHT-h) * 2,
		DLG_ROW4_Y	= DLG_OFFSET + (CTRL_HEIGHT-h) * 3,
		DLG_ROW5_Y	= DLG_OFFSET + (CTRL_HEIGHT-h) * 4,
		DLG_ROW6_Y	= DLG_OFFSET + (CTRL_HEIGHT-h) * 5,
		DLG_ROW7_Y	= DLG_OFFSET + (CTRL_HEIGHT-h) * 6,
		DLG_ROW8_Y	= DLG_OFFSET + (CTRL_HEIGHT-h) * 7,
		DLG_ROW_END	= DLG_ROW8_Y + (CTRL_HEIGHT),
	...
	CTRLDATA ctrl_data[]={ 
		...
		//加一个宫压较对
		{ CTRL_BUTTON, WS_VISIBLE | WS_TABSTOP, WS_EX_NONE,
			DLG_COLUMN3_X, DLG_ROW7_Y, DLG_COLUMN3_W, KEY_HEIGHT,
			IDC_FACTORY_MAINTAIN_TOCO_CHECK,
			STR_DLG_FACTORY_TOCORESET, 0, 0 },
			
		{ CTRL_STATIC,	WS_VISIBLE | SS_RIGHT,  WS_EX_NONE,
			DLG_COLUMN3_X, DLG_ROW8_Y + LABEL_OFFSET, DLG_COLUMN3_W, KEY_HEIGHT, 
	...
	static BUTTONDATA btn_toco_check = { 0 };
	...
	SetCtrlDataAddData2(ctrl_data, loops,
		IDC_FACTORY_MAINTAIN_TOCO_CHECK, (UINT32)&btn_toco_check);
	SetCtrlDataAddData2(ctrl_data, loops,
